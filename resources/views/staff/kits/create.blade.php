@extends('layouts.app')
@section('title', 'เพิ่มกระเป๋า')
@section('page-title', 'เพิ่มกระเป๋าปฐมพยาบาล')
@section('sidebar') @include('partials.sidebar-staff') @endsection

@section('content')
<div class="panel">
    <div class="panel-header d-flex align-items-center justify-content-between">
        <h5 class="mb-0"><i class="bi bi-plus-circle me-2 text-primary"></i>เพิ่มกระเป๋าปฐมพยาบาลใหม่</h5>
        <a href="{{ route('staff.kits.index') }}" class="btn btn-outline-secondary btn-sm"><i class="bi bi-arrow-left me-1"></i>ย้อนกลับ</a>
    </div>
    <div class="panel-body">
        <form action="{{ route('staff.kits.store') }}" method="POST">
            @csrf
            <div class="row g-3 mb-4">
                <div class="col-md-4">
                    <label class="form-label fw-bold">รหัสกระเป๋า *</label>
                    <input type="text" name="kit_code" class="form-control" required placeholder="เช่น KIT004" value="{{ old('kit_code') }}">
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-bold">ชื่อกระเป๋า *</label>
                    <input type="text" name="name" class="form-control" required placeholder="เช่น กระเป๋าปฐมพยาบาลชุดพิเศษ" value="{{ old('name') }}">
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-bold">คำอธิบาย</label>
                    <input type="text" name="description" class="form-control" placeholder="รายละเอียดหรือประเภทกิจกรรม" value="{{ old('description') }}">
                </div>
            </div>
            ...
            <div class="d-flex gap-2">
                <button type="submit" class="btn btn-primary"><i class="bi bi-check-lg me-1"></i>บันทึกกระเป๋า</button>
