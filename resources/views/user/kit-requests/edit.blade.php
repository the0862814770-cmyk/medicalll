@extends('layouts.app')
@section('title', 'แก้ไขคำร้องขอเบิกกระเป๋าปฐมพยาบาล')
@section('page-title')
    <div style="display:flex;align-items:center;gap:0.6rem;">
        <i class="bi bi-pencil-square" style="font-size:1.1rem;color:#6366f1;"></i>
        <span style="font-size:0.95rem;font-weight:700;color:#334155;">แก้ไขคำร้องขอเบิกกระเป๋าปฐมพยาบาล</span>
    </div>
@endsection
        ...
        <div class="card animate-in">
            <div class="card-header">
                <span class="card-header-title">
                    <i class="bi bi-pencil-square me-1" style="color:#6366f1;"></i>
                    แก้ไขคำร้องขอเบิกกระเป๋าปฐมพยาบาล
                </span>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('user.kit-requests.update', $kitRequest) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        @if(isset($kits) && $kits->count())
                            <select name="first_aid_kit_id" id="first_aid_kit_id" class="form-control" required>
                                ...
                                @foreach($kits as $kit)
                                    <option value="{{ $kit->id }}" {{ old('first_aid_kit_id', $kitRequest->first_aid_kit_id) == $kit->id ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="activity_name" id="activity_name" 
                               ...
                               value="{{ old('activity_name', $kitRequest->activity_name ?? '') }}"
                               required>
                        @error('activity_name')
                            <div class="form-error">{{ $message }}</div>
                        @enderror
                        ...
                        <div style="display:flex;gap:0.75rem;flex-wrap:wrap;">
                            <div style="flex:1;min-width:200px;">
                                ...
                                <input type="date" name="borrow_date" id="borrow_date" 
                                       class="form-control" 
                                       value="{{ old('borrow_date', $kitRequest->borrow_date?->format('Y-m-d')) }}"
                                       required>
                                @error('borrow_date')
                                    <div class="form-error">{{ $message }}</div>
                                @enderror
                            ...
                            <div style="flex:1;min-width:200px;">
                                ...
                                <input type="date" name="expected_return_date" id="expected_return_date" 
                                       class="form-control" 
                                       value="{{ old('expected_return_date', $kitRequest->expected_return_date?->format('Y-m-d')) }}"
                                       required>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="quantity" id="quantity" 
                               class="form-control" 
                               value="{{ old('quantity', $kitRequest->quantity ?? 1) }}" 
                               min="1" 
                               required>
                        @error('quantity')
                            <div class="form-error">{{ $message }}</div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="participants_count" id="participants_count" 
                               ...
                               value="{{ old('participants_count', $kitRequest->participants_count ?? '') }}"
                               min="1" 
                               required>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="purpose" id="purpose" 
                                  ...
                                  required>{{ old('purpose', $kitRequest->purpose) }}</textarea>
                        @error('purpose')
                            <div class="form-error">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div style="display:flex;align-items:center;justify-content:flex-end;gap:0.75rem;margin-top:1.5rem;">
                        <a href="{{ route('user.kit-requests.index') }}" class="btn btn-secondary" style="border-radius:10px;">
                            ยกเลิก
                        </a>
